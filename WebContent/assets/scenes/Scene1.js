...
class Scene1 extends Phaser.Scene {
    ...
    update() {
        ...
        this.fPlayer.body.setVelocity(this.playerXVelocity, this.playerYVelocity);

        this.handelUpKey();
        this.handelDownKey();
        this.handelLeftKey();
        this.handelRightKey();
        this.handelWallUpdate(); 
    ...
    handelUpKey() {
        if (this.key_UP.isDown && this.sendUp) {
            this.playerYVelocity = -this.defaultVelocity;
            this.sendPlayerUpdate();
            this.sendUp = false;

        }
        if (this.key_UP.isUp && this.sendUp === false) {
            this.playerYVelocity = this.key_DOWN.isDown ? this.defaultVelocity : 0; 
            this.sendUp = true;
            this.sendPlayerUpdate();
        }
    }
    handelDownKey() {
        if (this.key_DOWN.isDown && this.sendDown) {
            this.playerYVelocity = this.defaultVelocity;
            this.sendPlayerUpdate();
            this.sendDown = false;
        }
        if (this.key_DOWN.isUp && this.sendDown === false) {
            this.playerYVelocity = this.key_UP.isDown ? -this.defaultVelocity : 0;
            this.sendDown = true;
            this.sendPlayerUpdate();
        }
    }
    handelRightKey() {
        ...
        if (this.key_RIGHT.isDown && this.sendRight) {
            this.playerXVelocity = this.defaultVelocity;
            this.sendPlayerUpdate();
            this.sendRight = false;
        }
        if (this.key_RIGHT.isUp && this.sendRight === false) {
            this.playerXVelocity = this.key_LEFT.isDown ? -this.defaultVelocity : 0; 
            this.sendRight = true;
            this.sendPlayerUpdate();
        }
    }
    handelLeftKey() {
        if (this.key_LEFT.isDown && this.sendLeft) {
            this.playerXVelocity = -this.defaultVelocity;
            this.sendPlayerUpdate();
            this.sendLeft = false;
        }
        if (this.key_LEFT.isUp && this.sendLeft === false) {
            this.playerXVelocity = this.key_RIGHT.isDown ? this.defaultVelocity : 0; 
            this.sendLeft = true;
            this.sendPlayerUpdate();
        }
    }
    ...
    updateAllPlayers() {
        for (var [k, v] of allPlayers) {
            v.player.body.setVelocity(v.xPos, v.yPos);
        }
    }
    ...
    sendPlayerUpdate() {
        var updateMessage = { type: "MOVEMENT", playerId: playerId, xPos: this.playerXVelocity, yPos: this.playerYVelocity };
